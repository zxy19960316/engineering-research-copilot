...
function Test-PreparedTask(
    ...
    if (-not $sourceTaskId.StartsWith('M4.1-')) {
        $mismatches += ([string]$Task.task_id + ':source_task_id_invalid')
        return [object[]]$mismatches
    }
    $expectedTaskId = 'M4.2-' + $sourceTaskId.Substring(5)
    if ([string]$Task.task_id -ne $expectedTaskId) {
        $mismatches += ([string]$Task.task_id + ':task_id_mismatch')
    }
    if ([string]$Task.root_task_id -ne [string]$SourceTask.source_task_id) {
        $mismatches += ([string]$Task.task_id + ':root_task_id_mismatch')
    }
    if ([string]$Task.source_blind_id -ne [string]$SourceTask.blind_id) {
        $mismatches += ([string]$Task.task_id + ':source_blind_id_mismatch')
    }
    if ([string]$Task.root_blind_id -ne [string]$SourceTask.source_blind_id) {
        $mismatches += ([string]$Task.task_id + ':root_blind_id_mismatch')
    }
    $sourceBatchId = [string]$Task.source_batch_id
    if (-not $sourceBatchId.StartsWith('M4.1-BATCH-')) {
        $mismatches += ([string]$Task.task_id + ':source_batch_id_invalid')
        return [object[]]$mismatches
    }
    $expectedBatchId = 'M4.2-BATCH-' + $sourceBatchId.Substring(11)
    if ([string]$Task.batch_id -ne $expectedBatchId) {
        $mismatches += ([string]$Task.task_id + ':batch_id_mismatch')
    }
    if ([string]$Task.root_batch_id -ne [string]$SourceTask.source_batch_id) {
        $mismatches += ([string]$Task.task_id + ':root_batch_id_mismatch')
    }
...
$manifestPath = Resolve-RepoFile $repoRoot 'evals/m4/revisions/m4.2/preparation-manifest.json'
$sourceManifestPath = Resolve-RepoFile $repoRoot 'evals/m4/revisions/m4.1/preparation-manifest.json'
...
$sourceTasks = @{}
foreach ($sourceTask in $sourceManifest.tasks) {
